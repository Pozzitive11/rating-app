...
Тут ключова проблема: в URL — untappd id, а в БД — твій “внутрішній” id. Потрібно мати зв’язок між ними. Є 2 нормальні варіанти:
Варіант B (правильніше): мати таблицю beers з мапінгом
Створи таблицю beers з такими полями:
id — внутрішній
untappd_id — зовнішній
beer_reviews тоді зберігає beer_id (внутрішній), і ти робиш:
1) По untappd_id знаходиш beer_id
2) По beer_id + user_id шукаєш оцінку
Плюси: чиста модель, легко розширювати
Мінуси: трохи більше логіки
